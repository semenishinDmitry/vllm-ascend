...
class KernelComputeWy {
 ...
 private:
  ...
  __aicore__ inline void ProcessOneTask(uint32_t b, uint32_t kHeadIdx, uint32_t vHeadIdx, uint32_t chunkIdx) {
    ...
    // UploadP uses kHalf; U R-half uses vHalf; W R-half uses qHalf (no alias with P).
    // kFloat is free after BuildKBetaExpFloat — C scratch (tiled when V>K).
    for (uint32_t round = 0; round < DOUBLING_ROUNDS; ++round) {
      cubeGemm_.UploadP(attnLocal, kHalf);
      cubeGemm_.GemmApplyAdd(vFloat, vHalf, kFloat, chunkKElems_, vHeadDim_, alignV_, /*useU=*/true);
      cubeGemm_.GemmApplyAdd(kBeta, qHalf, kFloat, chunkKElems_, kHeadDim_, alignK_, /*useU=*/false);
      if (round + 1 < DOUBLING_ROUNDS) {
        cubeGemm_.GemmSquare(attnLocal, kHalf);
        SyncEvent<HardEvent::MTE2_V>(HardEvent::MTE2_V);
      }
    }
